...
public class BookPanel extends JPanel {
				...
				public void actionPerformed(ActionEvent e) {
					...
					if(tfTitle.getText().length()==0&&tfAuthor.getText().length()==0) {      // 도서명과 저자의 검색어가 없을경우.
						for(int i=0; i<MainData.getBooks().size(); i++) {                    // Books 리스트에 있는 객체를 전부 출력하기.
							...
							String BookStatus = "";
							if(MainData.checkOutCtrl.getCheckOutAble(b.getBookNo()).equals("대여가능")){BookStatus = "O";}
							else {BookStatus = "X";}
							
							String[] data = {b.getBookNo(),b.getTitle(), b.getAuthor(),BookStatus, b.getSort(), String.format("%s일",b.getDate())};
							model.addRow(data);}}
					 ...
					 // 검색어가 둘중 하나 이상 있는 경우.
					     ...
					     List<Book> list = MainData.bookC.search(findStr, findStr2);   // 검색어와 일치하는 해당 객체의 리스트를 불러오는 메소드.
								...
								for(int i=0; i<list.size(); i++) {       // 해당 객체들의 리스트를 출력.
									Book b = list.get(i);
									String DY = "";
									if(MainData.checkOutCtrl.getCheckOutAble(b.getBookNo()).equals("대여가능")){DY = "O";}
									else {DY = "X";}									
									String[] data = {b.getBookNo(),b.getTitle(), b.getAuthor(),DY, b.getSort(), String.format("%s일",b.getDate())};
									model.addRow(data);
								}}
			...
			btnNewButton_1_2.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (result == JOptionPane.YES_OPTION) {
						// 첫 번째 입력이 아닐 경우
						if (MainData.getBooks().size() != 0) {
							// 도서 존재 여부
							for(int i=0; i<MainData.getBooks().size(); i++) {    // Books에 있는 객체 모두 model에 출력하기.
								Book b1 = MainData.getBooks().get(i);
								if (b1.getTitle().equals(bTitle)) {
									isSame = true;
									break;
								}
							}
					...
					String msg = MainData.bookC.append(b, isSame);                  // 생성된 객체를 books 리스트에 저장
	...
	public void reTable() {
		...
		model.setRowCount(0);
		for (int j = 0; j < MainData.getBooks().size(); j++) {
			Book b = MainData.getBooks().get(j);
			String BookStatus = "";
			if(MainData.checkOutCtrl.getCheckOutAble(b.getBookNo()).equals("대여가능")){BookStatus = "O";}
			else {BookStatus = "X";}
			String[] data = { b.getBookNo(), b.getTitle(), b.getAuthor(),BookStatus, b.getSort(),String.format("%s일", b.getDate()) };
