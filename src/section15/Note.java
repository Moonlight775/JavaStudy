...
public class Note extends JFrame {
	...
	public void save() {
		try {							   //↓장치명
			FileWriter fw = new FileWriter(fileName); //보조스트림
	...
	public void load() { //파일선택-파일읽기
		try {
			JFileChooser fc = new JFileChooser(fileName);    //↓파일선택
			...
			if(flag == JFileChooser.APPROVE_OPTION) {
				...
				FileReader fr = new FileReader(fileName);   //↓읽기
				
				char[] buff = new char[(int)f.length()];
				fr.read(buff);								//↑
				textArea.setText(new String(buff));			//←view
			}
		}catch(Exception ex) {
			ex.printStackTrace();
			JOptionPane.showMessageDialog(Note.this, "파일 읽기중 오류 발생");
		}
	}
	public Note() {
		setTitle("Note");
	...
	public JMenuItem getMntmNewMenuItem() {
		if (mntmNewMenuItem == null) {
			mntmNewMenuItem = new JMenuItem("\uC0C8\uAE00");//새글
	...
	public JMenuItem getMntmNewMenuItem_1() {
		if (mntmNewMenuItem_1 == null) {
			mntmNewMenuItem_1 = new JMenuItem("\uC77D\uAE30");
